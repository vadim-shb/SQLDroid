 ...
 * <p>
 * This is still pretty ugly.  Part of the problem is an architectural decision to handle errors
 * within the JDBC driver.
 * <p>
 * ALL databases fail. Any code that used JDBC has to be able to handle those failures.
 * The locking errors in SQLite are analogous to network failures in other DBs  Handling them within
 * the driver deprives the upper level code of the opportunity to handle these in an application
 * specific manner.  The timouts in the first implementation were extreme.  The loops would wait one
 * second between attempts to retry that, like, the middle of next week in computer terms.
 * <p>
 * To provide a specific problem case for the timout code.  We routinely read data on sub-second
 * intervals. Frequently on a 100ms timebase.  The data collection and insert usually occur on the
 * same thread. This isn't a requirement, but it helps to have the code synchronized. If we attempt
 * an insert, we want the code to fail fast.  That insert is cached and will be completed when the
 * next insert is attempted. So 100ms later there will be two inserts (or 3,  200ms later, etc.).
 * <p>
 * This code now also makes the timeouts optional and attempts to minimize the performance penalties
 * in this case.
 */
public class SQLiteDatabase {
     ...
     *                      other than a locked exception or throws a locked exception after the
     *                      timeout has expired.
     */
    public SQLiteDatabase(String dbQname, long timeout, long retryInterval, int flags)
            throws SQLException {
        super();
    ...
    /**
     * Returns true if the exception is an instance of "SQLiteDatabaseLockedException".  Since this
     * exception does not exist in APIs below 11 this code uses reflection to check the exception
     * type.
     */
